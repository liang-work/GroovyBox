...
def watch_artists_with_albums() -> List[ArtistAlbums]:
    ...
    for artist_name, data in artists_dict.items():
        ...
        if safe_name:
            cnt = conn2.execute("SELECT COUNT(*) as c FROM tracks WHERE artist=? AND album IS NOT NULL", (safe_name,)).fetchone()["c"]
        else:
            cnt = conn2.execute("SELECT COUNT(*) as c FROM tracks WHERE (artist IS NULL OR artist='') AND album IS NOT NULL").fetchone()["c"]
